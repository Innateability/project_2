...
def review_objective(objective_id):
    ...
    auth = Authentication.query.get(session.get("user_id"))
    return render_template("employee_review.html", objective=objective, role="employee", state="employee",auth=auth)
...
def review_open_objective(objective_id,mode):
    ...
    auth = Authentication.query.get(session.get("user_id"))
    return render_template("admin_review.html", objective=objective, role="employee", state="employee",auth=auth, mode=mode, auth_reviewed=auth_reviewed)
...
def edit_review(objective_id,mode):
    ...
    if request.method == "POST":
        ...
        return redirect(url_for("employee.home", objective_id=objective.id, auth=auth, mode=""))
    auth = Authentication.query.get(session.get("user_id"))
    return render_template("employee_edit_review.html", objective=objective,review=review,state="employee",auth=auth,role="employee")
